...
class Region {
  ...
  boolean touched;
  int id; // 1 left, 2 right

  public Region(PApplet p, Rectangle r, Rectangle s, float f, int id) {
    parent = p;
  ...
  void drawBox(boolean flagMore) {
    // Draw the hotspot rectangle.
    pushStyle();
    if (touched) {
      stroke(255, 200, 0);
      if(flagMore) fill(0, 255, 0, 160);
      if(!flagMore) fill(255, 0, 0, 160);
    } else {
      stroke(160);
      noFill();
    }
